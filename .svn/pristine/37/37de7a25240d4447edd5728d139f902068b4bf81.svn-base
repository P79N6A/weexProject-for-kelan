...
<template>
	<div class="container">
		...
		<scroller style="flex: 1;">
			<buy-page @changeAcount="changeAcount" @nextPage="nextPage" @isAgreeProtocol="isAgreeProtocol" :AvailBal="AvailBal" :AcNameFirst="AcNameFirst" :depositType="depositType"></buy-page>
		</scroller>
...
<script>
	...
	export default {
		...
		created(){
			this.getToken();
			// this.rateItem = this.$store.state.rateItem;
			this.CommonPayerAcNoList = this.$store.state.CommonPayerAcNoList;
		...
		methods:{
			...
			nextPage(buyMoney){
				...
				if(parseFloat(buyMoney)<5000){
					this.toastMsg1('起购金额为5000元');
					return;
				}
				this.showPwd = true;
			...
			getPwd(pwd){
				...
				this.postDo("FestProductBuy.do",param,data=>{
